...
**클라이언트**  
  1. 스켈레톤 기반 채점 도입으로 선 두께 차이에 따른 점수 불일치 문제 해결
  2. 캔버스 크기 변경 시 스켈레톤 이미지 리사이즈로 크기 불일치(호환성) 버그 수정

**서버**  
  1. IP-Adapter + LoRA 학습 구조로 스타일 반영 성능 향상, 데이터 수집·정제 한계 존재  
  2. 윤곽선 추출 로직 최적화로 다양한 배경·노이즈 환경에서도 안정적 결과 도출   
...
사용자가 이름을 입력하고 원하는 스타일의 이미지를 업로드하면, AI가 입력한 이름에 스타일을 결합해 매번 다른 형태의 서명을 생성하고 저장할 수 있도록 하는 기능입니다.

이를 구현하기 위해서 먼저 AI 모델 선정이 필요했습니다.

#### 서명 생성 AI 모델 선정

이 서비스에서는 사용자의 "이름"을 입력받기 때문에 text-to-image 모델을 사용하고자 했습니다. 트렌드와 많이 사용되고 있는 모델 위주로 검토 후 테스트를 진행했고, 선정된 모델은 아래와 같았습니다.

- Flux
  - 가장 성능이 좋고 서명이 잘 생성됨
  - 스타일 적용에 약함
  - 굉장히 고성능의 GPU를 요구, DISK와 GPU의 하드웨어적 한계로 사용 불가 판단
 
- Stable Diffusion 2-1
  - 서명 생성에 특화되지 않아 이름을 입력해도 서명답지 않은 결과 생성
  - 업로드한 이미지의 스타일이 서명에 반영되지 않는 문제 발생

- IP-Adapter
  - Stable Diffusion 1-5를 기반으로 동작하여 성능은 준수함
  - 텍스트와 이미지 스타일을 함께 임베딩해 스타일 반영 가능
  - 스타일 적용이 강해서 이름 형태가 완전히 사라지는 문제가 있음
 

이 중에서 사용자가 업로드한 이미지의 스타일은 반영을 해야하므로 스타일 반영이 제일 강한 IP-Adapter를 도입하고, 이름 형태 유지를 위해 추가적인 학습이 필요하다고 판단했습니다.

학습을 위해 LoRA 방식을 적용했습니다. 약 1,000장의 데이터를 수집하고 크기, 기울기, 노이즈 조절 등으로 증강한 뒤 IP-Adapter에 학습시켰습니다. 결과를 확인해보니 스타일 반영은 개선되었지만, 여전히 일부 서명은 이름 인식이 어렵거나 낙서처럼 보이는 한계가 있었습니다. 데이터로 사용된 서명 이미지들의 스타일 일관성으로 인한 문제로 판단했고, 개선하고자 했으나 데이터 양은 매우 많이 필요했지만 그만한 데이터들이 웹 상에 존재하지 않아 데이터 확보에 한계가 있었습니다. 충분한 양의 정제된 서명 데이터를 확보할 수 있다면, LoRA 단독 학습만으로도 서명 생성 품질을 크게 향상시킬 수 있을 것으로 예상됩니다. 현재 기술적 과제는 모델 구조보다는 데이터 확보와 정제에 있으며, 이 부분이 해결된다면 실용적인 수준의 서명 생성 기능 구현이 가능할 것으로 보입니다. 

> LoRA학습은 기존 대규모 모델의 모든 파라미터를 재학습하지 않고, 특정 레이어에 소규모 가중치(저랭크 어댑터)를 추가·학습해 모델이 새로운 데이터나 스타일을 빠르고 효율적으로 학습할 수 있도록 하는 방식입니다.
...
이 기능을 구현하면서 겪었던 기술적 챌린지는 윤곽선 추출 과정에서 일부 선이 사라지는 현상이었습니다. Canny 알고리즘을 사용해 윤곽선을 검출할 때, 서명 선이 얇거나 흐릴 경우 제대로 검출되지 않는 문제였습니다.
